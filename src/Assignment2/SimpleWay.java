...
/**
 * The SimpleWay class has multiple checks to fit a specific criteria
...
public class SimpleWay extends Airline {
	...
	private Seat s_temp = null;
	private Seat s_temp2 = null;
	 ...
	 * it books a middle or window seat reserving an additional
	 * seat to the left or right.
	 * @param aFlight the picked flight
	 * @param aType the requested seat type
	 * @return a seat object with a seat or null depending on situation
	 */
	...
	public Seat firstClassEcon(Flight aFlight, SeatType aType)
	{
		for (int c = 0; c <= aFlight.getSeating().getnColumns() - 1; c++)
		{
			for (int r = 0; r <= aFlight.getSeating().getnRows() - 1; r++)
			{
				...
				{
					if (aFlight.getSeating().getSeatsArray()[c][r].equals(aFlight.getSeating().queryAvailableEconomySeat(SeatType.WINDOW))) 
					{
						s_temp = aFlight.getSeating().getSeatsArray()[c][r];
						s_temp2 = aFlight.getSeating().getLeft(aFlight.getSeating().getSeatsArray()[c][r]);
						if (s_temp2 == null) 
						{
							s_temp2 = aFlight.getSeating().getRight(aFlight.getSeating().getSeatsArray()[c][r]);
						}
						if (s_temp2 != null && !s_temp2.isReserved()) 
						{
